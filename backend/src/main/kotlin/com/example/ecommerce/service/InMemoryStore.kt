...
class InMemoryStore {
    ...
    constructor(orderEventPublisher: OrderEventPublisher) {
    ...
    fun listOrders(page: Int?, size: Int?, userId: String?, sortBy: String?, sortOrder: String?): PageResponse<Order> {
        val pageNumber = normalizePage(page)
        val pageSize = normalizeSize(size)
        val filtered = orders.values.filter { userId.isNullOrBlank() || it.userId == userId }
        val comparator = when (sortBy?.lowercase()) {
            "totalamount" -> compareBy<Order> { BigDecimal(it.totalAmount) }
            "userid" -> compareBy<Order> { it.userId }
            else -> compareBy<Order> { it.createdAt }
        }
        return paginate(applySorting(filtered, sortOrder, comparator), pageNumber, pageSize)
    }
    ...
    fun createOrder(payload: OrderPayload): Order {
        ...
        val submittedItems = payload.items.orEmpty().map { item ->
            ...
            orderItems[orderItemId] = OrderItem(orderItemId, order.id, productId, quantity, price, timestamp, timestamp)
    ...
    fun updateOrder(id: String, payload: OrderPayload): Order {
        val current = orders[id] ?: throw NotFoundException("订单不存在")
    ...
    fun deleteOrder(id: String) {
        ...
        orderItems.entries.removeIf { it.value.orderId == id }
        payments.entries.removeIf { it.value.orderId == id }
    }
